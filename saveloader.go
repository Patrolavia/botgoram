...
type SaveLoader interface {
	Save(uid int, sid string, data interface{}) error
	Load(uid int) (sid string, data interface{}, err error)
}
...
func (m *memoryStore) Save(uid int, sid string, data interface{}) error {
	m.data[uid] = data
	m.state[uid] = sid
	return nil
}

func (m *memoryStore) Load(uid int) (sid string, data interface{}, err error) {
	data, ok := m.data[uid]
	sid = m.state[uid]
	if !ok {
		data = m.init(uid)
